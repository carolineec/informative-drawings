...
class Visualizer():
    def __init__(self, checkpoints_dir, name, tf_log=True, isTrain=True, no_html=True, display_winsize=512):
        # self.opt = opt
    ...
    def display_current_results(self, visuals, epoch, step):
        ...
        if self.tf_log: # show images in tensorboard output
            ...
            for label, image_numpy in visuals.items():
                ...
                # disp_image = np.expand_dims(image_numpy, axis=0)
                # print(image_numpy.shape, 'HEY', label, image_numpy.shape)
                # self.tf.summary.image(label, disp_image, step=step)
                # self.writer.flush()
